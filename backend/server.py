...
# Connect to the local database, containing vehicles generated by database_generator.py
conn = sqlite3.connect("backend/sixt.db", check_same_thread=False)
cursor = conn.cursor()

# Real life sixt station locations based on their latitudes and longitudes.
locations = {
...
# Helper function to create a dictionary object from the database queries
...
# Alternative distance calculation method based on euclidean distance
# Note: for close proximities, we can assume the world is flat and this can be accurate.
...
# Preferred distance calculation metehod based on haversine distance
# Note: takes account of the world's curvature for a more accurate calculation
...
# Get all vehicles in a particular location
...
# Get the distance to all Sixt stations based with the current coordinate as its origin
...
# Get all available vehicles in a particular location
...
def get_upsell(booked_id, lat, long):
    ...
    # Calculate distance from the current location to the available stations
    distance_to_stations = calculate_haversine(current_coords, locations)
    
    # Get the currently booked vehicle
    booked_vehicle = get_vehicles(booked_id)
    
    
    # Since we want to upsell, we query all available vehicles above the current price bracket
    ...
    # Compute similarity based on cosine similarity between the currently booked vehicles and the potential vehicles to be upsold
    id_similarity = {}
    for vehicle in available_vehicles_upsell:
        id_similarity[vehicle['id']] = _similarity_scoring(booked_vehicle, vehicle)
    
    # Get the highest similarity score for normalization purposes
    max_similarity = max(id_similarity.values())
    
    # We sort the stations based on the distance from the user
    sorted_station = sorted(distance_to_stations.items(), key=lambda x: x[1])
    
    station_cluster = {}
    station_id_score = {}
    
    # For each station, we return a sorted list of vehicle IDs based on their similarity score
    ...
    # We prepare our final upsell recommendation list by only considering cars with similarities above 75%
    station_id_score = {}
    final_ids = []
    loc_counter = 0
...
def _similarity_scoring(vehicle_1, vehicle_2):
    # We transform the attributes of the vehicles into numerical vector
    ...
    # Based on a distribution, we put higher emphasis on vehicles that are +1-10 euros per day
